...
export function NotebookActivities({ lesson, courseTitle, onOpenNotesWidget }: NotebookActivitiesProps) {
  ...
  // Generate tailored notebook activities for the lesson
  const notebookItems: NotebookItem[] = React.useMemo(() => {
    const verseRef = lesson.baseVerse?.reference || 'el pasaje bíblico base';
    const mainTitle = lesson.title || 'esta lección';

    return [
      {
        id: `${lesson.id}_note_1`,
        category: 'APUNTE',
        title: 'Esquema Sintético a Mano',
        instruction: `Abre tu libreta de estudio, coloca la fecha y el título "${mainTitle}". Traza un cuadro de 3 columnas para organizar tus ideas:`,
        hint: `Columna 1: Verdad Teológica Principal • Columna 2: Citas Bíblicas Clave (${verseRef}) • Columna 3: Implicación para tu Fe.`
      },
      {
        id: `${lesson.id}_question_1`,
        category: 'PREGUNTA',
        title: 'Pregunta de Autoexamen Personal',
        instruction: `Responde por escrito en tu cuaderno: ¿De qué manera la verdad expuesta en "${mainTitle}" confronta o fortalece mi vida diaria y mi relación con Dios?`,
        hint: 'Escribe al menos un párrafo honesto expresando tus reflexiones personales.'
      },
      {
        id: `${lesson.id}_question_2`,
        category: 'PREGUNTA',
        title: 'Análisis de Pasaje en Libreta',
        instruction: `Transcripción y Marcado: Escribe a mano el versículo principal (${verseRef}) en tu libreta. Subraya con color las palabras teológicas clave (ej. fe, gracia, justicia, reino) y anota flechas de conexión al margen.`,
        hint: 'El acto de transcribir a mano fija los conceptos bíblicos en la memoria a largo plazo.'
      },
      {
        id: `${lesson.id}_activity_1`,
        category: 'ACTIVIDAD',
        title: 'Ejercicio Práctico en Papel',
        instruction: `Diseña un mapa conceptual o diagrama de flujo en tu cuaderno que muestre cómo aplicar el tema central de hoy en tu ministerio, familia o comunidad.`,
        hint: 'Puedes dibujar flechas conectando la Doctrina ➔ La Mente ➔ El Corazón ➔ La Acción Práctica.'
      },
      {
        id: `${lesson.id}_activity_2`,
        category: 'ACTIVIDAD',
        title: 'Oración Escrita de Cierre',
        instruction: `Redacta una oración a mano en la parte inferior de tu hoja, pidiendo al Señor gracia para vivir de acuerdo con lo aprendido en "${mainTitle}".`,
        hint: 'Conserva esta oración en tu cuaderno para revisar tus respuestas de fe con el tiempo.'
      }
    ];
  }, [lesson]);
